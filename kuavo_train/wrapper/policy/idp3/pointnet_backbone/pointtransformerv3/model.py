...
def segment_csr_fallback(src: torch.Tensor, indptr: torch.Tensor, reduce: str) -> torch.Tensor:
    """torch_scatter.segment_csr fallback using torch.segment_reduce.

    Args:
        src: [N, C] input tensor sorted by segment id.
        indptr: [num_segments + 1] CSR pointer.
    ...
    if not hasattr(torch, "segment_reduce"):
        raise ImportError(
            "PTv3 requires either `torch_scatter` or PyTorch with `torch.segment_reduce` support."
        )
    if reduce not in {"sum", "mean", "min", "max"}:
        raise ValueError(f"Unsupported reduce mode: {reduce}")

    return torch.segment_reduce(src, reduce=reduce, offsets=indptr.long())
...
class SerializedPooling(PointModule):
    ...
    def forward(self, point: Point):
        ...
        # indices of point sorted by cluster, for segment CSR reduction
        _, indices = torch.sort(cluster)
        # index pointer for sorted point, for segment CSR reduction
        ...
        # collect information
        point_dict = Dict(
            feat=segment_csr_fallback(self.proj(point.feat)[indices], idx_ptr, reduce=self.reduce),
            coord=segment_csr_fallback(point.coord[indices], idx_ptr, reduce="mean"),
